...
class ContentDetails extends StatelessWidget {
  ...
  ContentController controller = Get.find<ContentController>();
  ...
  ContentDetails({required Content data, int this.index = -1}) {
  ...
  refresh() async {
    loading.value = true;
    Content? tmp = await controller.find({'id': data.value.id});
    if (tmp != null) {
      this.data.value = tmp;
    }
    loading.value = false;
